...
class CutskyCatalogBlinding(BaseClass):
    ...
    def png(self, data_positions, data_weights=None, randoms_positions=None, randoms_weights=None, method='randoms_weights',
        ...
        if recon.mpicomm.rank == 0: print(np.std(np.linalg.norm(shifts, axis=1)))
        ...
        if shotnoise_correction:
            csum_data_weights = mpy.csize(data_positions) if data_weights is None else mpy.csum(data_weights)
            csum_randoms_weights = mpy.csize(randoms_positions) if randoms_weights is None else mpy.csum(randoms_weights)
        ...
        if shotnoise_correction:
            k_pivot, mu_pivot = 8e-3, 0.6  # compute the correctif factor at k_pivot
